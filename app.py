...
from src.kinematics import calculate_prediction_cone
import pandas as pd
import numpy as np
import folium
import joblib
...
def generate_tactical_data(n=50):
    ...
    data = pd.DataFrame({
        ...
        "bearing": np.random.uniform(0, 360, n),
...
show_prediction = st.sidebar.checkbox("Show Intercept Cones", value=True)
time_horizon = st.sidebar.select_slider("Prediction Window", options=[30, 60, 120, 300], value=60)
st.sidebar.caption("Projected position based on current velocity vector.")
# =========================
# 4.5 APPLY FILTERS 
# =========================
...
for _, row in filtered_df.iterrows():
    ...
    # --- START OF PREDICTION CODE ---
    if show_prediction and row['is_intruder'] == 1: # We only predict for intruders
        cone_coords = calculate_prediction_cone(
            row.lat, row.lon, row.speed, row.bearing, time_horizon
        )
        folium.Polygon(
            ...
            fill_opacity=0.2,
            weight=1,
            tooltip=f"Intercept Zone ({time_horizon}s)"
        ).add_to(m)
